...
### 이유
1. **MUI 경험 활용**: 현재 MUI sx prop을 주로 사용 중이라 문법이 익숙함
2. **빠른 개발 속도**: 러닝커브 없이 바로 생산성 높게 개발 가능
3. **학습 목적 우선**: 라이브러리 최적화보다 빠른 구현과 학습이 목표
4. **CSS-in-JS 편의성**: 동적 스타일링, props 기반 스타일 변경 용이
5. **마이그레이션 가능**: 나중에 Vanilla Extract로 전환 가능한 구조

### 다른 선택지
1. **CSS Modules**
   - 장점: 제로 런타임, 번들 크기 0KB
   - 단점: 동적 스타일링 불편, 별도 파일 관리 필요

2. **Vanilla Extract**
   - 장점: 제로 런타임, TypeScript로 CSS 작성, 타입 안전
   - 단점: 러닝커브 있음, 설정 필요

3. **Tailwind CSS**
   - 장점: 빠른 개발, 유틸리티 클래스
   - 단점: 커스터마이징 제한, 라이브러리에 종속성 추가

4. **순수 CSS**
   - 장점: 의존성 없음, 가벼움
   - 단점: 스코핑 없음, 동적 스타일링 불편

### 트레이드오프
- ✅ 장점: 익숙한 문법, 빠른 개발, 동적 스타일링, 같은 파일에서 관리
- ❌ 단점: 런타임 오버헤드 (~10KB), 라이브러리 사용자에게 Emotion 의존성 추가
- 💡 우리 선택: **학습 목적**이므로 개발 속도 우선, 최적화는 나중에

### 마이그레이션 계획
프로젝트 완성 후 성능 최적화가 필요하면:
1. Emotion → Vanilla Extract로 스타일만 교체
2. 컴포넌트 로직은 그대로 유지
3. 같은 파일 내 스타일 정의로 마이그레이션 용이하게 구조화
...
| **패키지 매니저** | pnpm | 빠름, 디스크 효율, 안전성 | 일부 도구 호환성(드뭄) |
| **아키텍처** | Re-export 레이어 | 개발/사용 편의성, 의존성 분리 | 패키지 수 증가 |
| **스타일링** | Emotion | MUI 경험 활용, 빠른 개발 | 런타임 오버헤드 |
...
**작성일**: 2025-11-07
**최종 업데이트**: 2025-11-26 (Emotion 스타일링 결정 추가)
**다음 업데이트**: 패턴 구현 진행하며 경험 기록 예정
